...
(function () {
	...
	Ma.prototype = {
		...
		render : function (a,
			...
			if (m && r)
				t === "inside" && (r = -r), d.opposite && (r = -r), b = this.getMarkPath(H, E, r, m * C, g, f), p ? p.animate({
					d : b,
					opacity : c
	...
	eb.prototype = {
		...
		setOptions : function (a) {
			this.options = x(this.defaultOptions, this.isXAxis ? {}
					 ...
					 : this.defaultYAxisOptions, [this.defaultTopAxisOptions, this.defaultRightAxisOptions, this.defaultBottomAxisOptions, this.defaultLeftAxisOptions][this.side], x(L[this.isXAxis ? "xAxis" : "yAxis"], a))
		},
		update : function (a,
			b) {
			var c = this.chart,
		...
		render : function () {
			...
			if (B || f)
				if (a.minorTickInterval && !a.categories && n(a.getMinorTickPositions(), function (b) {
						...
						p && n(g, function (b, c) {
							if (c % 2 === 0 && b < a.max)
								l[b] || (l[b] = new vb(a)), s = b + q, w = g[c + 1] !== v ? g[c + 1] + q : a.max, l[b].options = {
									from : e ? fa(s) : s,
									to : e ? fa(w) : w,
									color : p
								},
	...
	fb.prototype = {
		...
		scroll : function (a, b) {
			...
			d > c && (d =
					c);
			if (d > 0)
				b !== v && La(b, this.chart), this.nav.attr({
					translateX : i,
	...
	yb.prototype = {
		...
		drawChartBox : function () {
			...
			if (k)
				f ? f.animate(t) : this.plotBackground = b.rect(q, o, n, r, 0).attr({
						fill : k
			...
			if (m)
				g ? g.animate(g.crisp(null, q, o, n, r)) : this.plotBorder = b.rect(q, o, n, r, 0, -m).attr({
			...
			if (a.visible && (e.enabled || a._hasPointLabels)) {
				...
				if (ua(D) === 0 || this.verifyDataLabelOverflow(D))
					this.placeDataLabels(), u && g && n(this.points, function (b) {
						...
						if ((s = b.dataLabel) &&
							s._pos)
							E = s._attr.visibility, z = s.connX, A = s.connY, j = k ? ["M", z + (w[6] === "left" ? 5 : -5), A, "C", z, A, 2 * w[2] - w[4], 2 * w[3] - w[5], w[2], w[3], "L", w[4], w[5]] : ["M", z + (w[6] === "left" ? 5 : -5), A, "L", w[2], w[3], "L", w[4], w[5]], i ? (i.animate({
									d : j
